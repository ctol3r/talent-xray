...
- UPDATE the existing requirement a statement is about; never add a second requirement alongside it saying the same thing in the manager's words. One concept, one RequirementIR.
- One requirement, one source phrase (W12 S-10). "statement" is the fragment of the turn that asserts THIS requirement, quoted exactly — not the whole turn. A manager who covers four things in one answer gives you four different statements, one per requirement; join split fragments with an ellipsis. Two requirements must never carry the same statement, and pasting the entire turn onto each of them destroys the provenance the field exists for.
- When the manager says something can be taught or learned on the job — "we can teach them our product", "propellant handling they'll pick up in a month" — record it as a requirement with kind "trainable". That is what the kind is for, and it is the only thing that stops the item being screened on.
...
- "consequential" means the answer would change WHO you approach or WHETHER they could accept (W12 S-9): the population, the geography, the reachable supply, or a material term — pay, relocation support, shift, start date, work authorisation. A question that only sharpens how you word a screen is worth asking but is NOT consequential, and neither is a process question like who signs off, unless the answer changes the requirement set. Be honest in both directions: over-flagging turns the next-question ranking into noise, and under-flagging hides the questions that decide whether the search is reachable at all.
...
Constructs and proxies (W12 S-7):
- When the hiring manager defines a term, their own defining words go in "definition" — "writes their own menus and changes them with what the valley produces", "opened seventy percent of their own pipeline". A definition that paraphrases the construct away leaves every downstream agent guessing at exactly the thing that was just settled.
- Prestige, brand and credential proxies — a named employer, a university, a ranking, an award, a certification usually earned after the work — are never left as bare requirements. Name the CONSTRUCT the proxy stands for in "definition" and put the proxy itself in "falseSignals" BY NAME — "TSMC on a profile used in place of the module experience", not "a company name used as a stand-in". A screener can act on the name and cannot act on the abstraction, and both directions belong there: the proxy present, read as evidence, and the proxy absent, read as a reason to deprioritise. A proxy with neither behind it is read downstream as a filter, which is how a prestige preference becomes a screen nobody chose.
