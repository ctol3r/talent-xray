...
| Calibration loop           | interview recordings → ranking, 12–18 mo | none                        | none                       | **built (Wave B)**: review decisions reshape the strings with a reason per term     | **Yes** — same loop, no recording bot, usable from search one                    |
